...
public class Multitenants {
    ...
    public static void main(String[] args) {
        ...
        countProbabilityInEachDatabase(dataCenter1,true);   
    ...
    public static DataCenter generateDBasesAndTenants(DataCenter dc){ 
        ...
        //dc.set_TenantInstances(tenantInstances);
            ...
            for(int j=0;j<20;j++){
                ...
                while(status){
                    ...
                    if(k==randomTenant.size()){
                        randomTenant.add(a);
                        dc.getDataBases().get(i).add_tenant(tenantInstances[a]);
                        status=false;
                    }                
                }
                //b= (int) (Math.random()*3);                
                //tenantInstances[k]=new Tenant(k, BizType.values()[a], TenantSize.values()[b]);               
                //dc.getDataBases()[i].add_tenant(tenantInstances[k]);
                //System.out.println(tenantInstances[k].get_Id()+ " "+tenantInstances[k].get_businessType()+" "+tenantInstances[k].get_tSize());                
                //k++;
            }     
        } */
